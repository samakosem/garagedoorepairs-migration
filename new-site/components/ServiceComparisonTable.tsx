...
export default function ServiceComparisonTable() {
  return (
    <section className="mx-auto max-w-6xl px-4 py-16">
      ...
      <div className="mt-8 hidden overflow-x-auto rounded-xl border border-zinc-200 shadow-sm sm:block">
      ...
      {/* Mobile: stacked cards, same data, no table */}
        ...
        {rows.map((row) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <p className="font-semibold text-brand-navy">{row.problem}</p>
              <span
                className={`flex-shrink-0 rounded-full px-2.5 py-1 text-xs font-medium ${urgencyColor[row.urgency]}`}
              >
                {row.urgency}
              </span>
            </div>
            <p className="mt-2 text-sm text-zinc-600">
              <span className="font-medium text-zinc-500">Likely Cause: </span>
              {row.cause}
            </p>
            <Link
              href={row.servicePath}
              className="mt-3 flex items-center justify-center rounded-md bg-brand-blue px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-brand-navy"
            >
              {row.service}
            </Link>
          </div>
